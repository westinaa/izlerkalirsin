...
const westinadb = (global.westinasetupxd = new Database("./settings/configs/sunucuayar.json"));
...
(async () => {
try {
  console.log('[westina] Slash ve Komutlar yükleniyor.');
  await rest.put(
    Routes.applicationGuildCommands(allah.Main.BotClientID, allah.GuildID),
    { body: slashcommands },
  ).then(() => {
    console.log('[westina] Slash ve Context Komutlar yüklendi.');
  });
}
catch (e) {
  console.error(e);
...
const rakam = client.sayıEmoji = (sayi) => {
  var westinacim = sayi.toString().replace(/ /g, "     ");
  var westinacim2 = westinacim.match(/([0-9])/g);
  westinacim = westinacim.replace(/([a-zA-Z])/g, "Belirlenemiyor").toLowerCase();
  if (westinacim2) {
    westinacim = westinacim.replace(/([0-9])/g, d => {
      return {
