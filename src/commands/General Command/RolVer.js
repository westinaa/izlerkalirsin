...
module.exports = {
  conf: {
    ...
    name: "rolver",
    help: "rolver <westina/ID> <Role/ID>",
    category: "yetkili",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    {
    message.reply({ content:`Bu komutu kullanmak için yetkiniz bulunmamakta..`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    return }
    ...
    let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2])
        ...
        if (args[0] == "al") {
          if (rMember.roles.cache.has(role.id)) {
            ...
            message.reply({ embeds: [embed.setThumbnail(message.author.avatarURL({dynamic: true, size: 2048})).setDescription(`${rMember} Kişisinden ${role} rolünü aldım.`).setColor("#ffffff")]})
          } else {
            message.reply({ embeds: [embed.setThumbnail(message.author.avatarURL({dynamic: true, size: 2048})).setDescription(`${rMember} Kişisinde ${role} rolü mevcut değil.`).setColor("#ffffff")]})
          }
      }
      if (args[0] == "ver") {
          if (!rMember.roles.cache.has(role.id)) {
            rMember.roles.add(role.id)
            message.reply({ embeds: [embed.setThumbnail(message.author.avatarURL({dynamic: true, size: 2048})).setDescription(`${rMember} Kişisine ${role} rolünü ekledim.`).setColor("#ffffff")]})
          } else {
            message.reply({ embeds: [embed.setThumbnail(message.author.avatarURL({dynamic: true, size: 2048})).setDescription(`${rMember} Kişisinde ${role} rolü zaten mevcut.`).setColor("#ffffff")]})
          }
      }
   },
 };
