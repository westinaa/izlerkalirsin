...
.setEmoji("⏭️"),
        ...
        if (liste.length <= 10) {
        await message.channel.send({ embeds: [new EmbedBuilder().setColor("#ffffff").setDescription(`${liste.slice(page == 1 ? 0 : page * 10 - 10, page * 10).join('\n')}`).setTimestamp().setAuthor({ name: Member.user.tag, iconURL: Member.user.avatarURL({ dynamic: true  })})]});
        } else if (liste.length > 10) {
        var msg = await message.channel.send({ embeds: [new EmbedBuilder().setColor("#ffffff").setDescription(`${liste.slice(page == 1 ? 0 : page * 10 - 10, page * 10).join('\n')}`).setTimestamp().setAuthor({ name: Member.user.tag, iconURL: Member.user.avatarURL({ dynamic: true  })})], components: [row]});
        }
       ...
       if(button.customId === "önce") {
        await button.deferUpdate();
                    ...
                    msg.edit({ embeds: [new EmbedBuilder() .setColor("#ffffff").setDescription(`${rollogVeri}`).setTimestamp().setAuthor({ name: Member.user.tag, iconURL: Member.user.avatarURL({ dynamic: true  })})]});
                }
