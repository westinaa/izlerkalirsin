...
const westinacim = require("../../../config.js");

module.exports = {
    conf: {
        aliases: ["seslog", "slog"],
        name: "slog",
        help: "slog <westina/ID>",
        category: "owner",
    },

    run: async (client, message, args, embed, prefix) => {
        if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !westinacim.owners.some(x => message.member.roles.cache.has(x))) {
        ...
        const Veri = await kanallar.findOne({ userId: String(Member.id), guildId: message.guild.id });

        console.log(`Veritabanından gelen veri:`, Veri);

        if (!Veri || !Veri.channelData || Veri.channelData.length === 0) {
            return message.reply({ content: `<@${Member.id}> kişisinin kanal bilgisi veritabanında bulunamadı.` });
        }
