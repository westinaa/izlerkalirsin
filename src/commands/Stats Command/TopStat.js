...
run: async (client, message, args, prefix) => {
  ...
  collector.on("collect", async (interaction) => {
    if (interaction.values[0] === "top1") {
      ...
      const mesajeniyi = messageUsersData1.splice(0, 1).map((x) => `<@${x.userID}>`);
      const seseniyi = voiceUsersData1.splice(0, 1).map((x) => `<@${x.userID}>`);
      ...
      const mesajhaftanıneniyisi = messageUsersData2.splice(0, 1).map((x) => `<@${x.userID}>`);
      const seshaftanıneniyisi = voiceUsersData2.splice(0, 1).map((x) => `<@${x.userID}>`);
...
.map((x, index) => `${x.userID === message.author.id ? `\` ${index+1} \` <@${x.userID}> \`${moment.duration(x.topStat).format("H [saat], m [dakika]")}\` **(Sen)**` : `\` ${index+1} \` <@${x.userID}> \`${moment.duration(x.topStat).format("H [saat], m [dakika]")}\``}`)
.join("\n");
  // Eğer list boşsa, "Veri Bulunmuyor." mesajını ekle
