...
module.exports = {
  ...
  .setDisabled(true),
    ...
    if (message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      ...
      message.react(green)
      message.reply({ embeds: [embed.setThumbnail(message.author.avatarURL({ dynamic: true, size: 2048 })).setColor("#7fff00").setDescription(`${message.author}, ${member} kişisini başarılı bir şekilde yanınıza taşıdınız.`).setFooter({ text: `${moment(Date.now()).format("LLL")}`})] });
      const log = embed
        .setColor("#ffffff")
        .setDescription(`
      Bir \` .çek \` işlemi gerçekleşti.
    
      Taşınan Kullanıcı: ${member} - \`${member.id}\`
      Taşıyan Kullanıcı: ${message.author} - \`${message.author.id}\``)
      ...
      collector.on("collect", async (button) => {
        ...
        if (button.customId === "onay") {
          ...
          const embeds = new EmbedBuilder()
            ...
            .setTimestamp()
            .setDescription(`${message.author}, ${member} kişisini başarıyla yanınıza taşıdınız.`)
          member.voice.setChannel(message.member.voice.channel.id);
        ...
        if (button.customId === "red") {
          ...
          const embedss = new EmbedBuilder()
            ...
            .setFooter({ text: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true })})
            .setTimestamp()
            .setDescription(`${message.author}, ${member} yanına taşıma işlemi kişi tarafından reddedildi.`)
