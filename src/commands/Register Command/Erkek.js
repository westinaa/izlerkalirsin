...
run: async (client, message, args, embed, prefix) => { 
    ...
    message.reply({ embeds: [new EmbedBuilder()
      ...
      .setThumbnail()
      .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
      .setDescription(`${red} Kayıt edilecek kullanıcıyı etiketlemelisiniz.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
    return }
    ...
    message.reply({ embeds: [new EmbedBuilder()
      ...
      .setThumbnail()
      .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
      .setDescription(`${red} Kullanıcı kayıt edilemiyor.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
    return }
    ...
    message.reply({ embeds: [new EmbedBuilder()
      ...
      .setThumbnail()
      .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
      .setDescription(`${red} Senden yüksek yetkideki birini kaydedemezsin.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
    return }
    ...
    message.reply({ embeds: [new EmbedBuilder()
      ...
      .setThumbnail()
      .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
      .setDescription(`${red} Kayıt edeceğiniz kullanıcıyı etiketlemelisiniz.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
    return }
...
let erkekemdes = new EmbedBuilder()
  ...
  .setColor("#ffffff")
  .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
  .setDescription(`${uye.toString()} isimli üye ${message.author} isimli yetkili tarafından **ERKEK** olarak \`${message.guild.name}\` sunucusuna kayıt edildi.`)
