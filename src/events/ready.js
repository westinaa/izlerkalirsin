...
setInterval(async () => {  
 ...
 // console.log("conf.chatMute içeriği:", conf.chatMute); 
  ...
  activePenals.forEach(async (x) => {
    const member = guild.members.cache.get(x.userID);
    if (!member) return;
    if (Array.isArray(conf.chatMute) && conf.chatMute.some((x) => member.roles.cache.has(x))) {
      return member.roles.add(conf.chatMute);
    } else {
      console.error("conf.chatMute bir dizi değil veya tanımlanmamış! Değeri:", conf.chatMute);
    }
...
const guild = client.guilds.cache.get(allah.GuildID);
let westina = guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== guild.id).size == 0)
westina.forEach(r => {
