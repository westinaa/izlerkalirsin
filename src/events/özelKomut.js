    ...
    if (message.member.roles.cache.some(rol => komut.YetkiliRol.includes(rol.id)) || message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        if (!üye) {
            return message.reply({ 
                embeds: [new EmbedBuilder().setColor("#ff0000").setDescription(`${red} Lütfen rol verilecek kişiyi etiketle.`)] 
            }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        }
        ...
        if (!komut.verilecekRol.some(rol => üye.roles.cache.has(rol))) {
            ...
            message.channel.send({ 
                embeds: [new EmbedBuilder().setColor("#00ff00").setDescription(`${green} Başarılı şekilde ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü verdim.`)]
            }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            
            if (channel) {
                channel.send({ 
                    embeds: [new EmbedBuilder().setDescription(`${message.member} yetkilisi tarafından ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolü verildi.`)]
                });
            }
        } else {
            ...
            message.react(green);
            message.channel.send({ 
                embeds: [new EmbedBuilder().setColor("#ff0000").setDescription(`${green} Başarılı şekilde ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü aldım.`)]
            }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            
            if (channel) {
                channel.send({ 
                    embeds: [new EmbedBuilder().setDescription(`${message.member} yetkilisi tarafından ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolü alındı.`)]
                });
            }
        }
    }
